    ...
    <script type="text/javascript">
		...
		//   end of var popup = new gomp.Popup
...
//----------------------------------
		...
		//-----------map.addLayer(-------------				
                ...
                map.addLayer({
					...
					//  --------- end of  while (Math.abs(e.lngLat.lng - coordinates[0]) > 180)
                    // 設定訊息視窗內容並在地圖上顯示
                    popup.setLngLat(coordinates)
                        .setHTML(description)
                        .addTo(map);
                });		//   end of  map.on('click', 'circles', function (e) {
                ...
                document.getElementsByName('switch').forEach((input) => {
                    input.addEventListener("change", function (event) 
					{
                       ...
                       map.setFilter('circles', ['==', 'species', val]);
                        ...
                        // 加上底下這一行
                        popup.remove();
                    })		//    end of  input.addEventListener("change", function (event) 
                });		//   end of   map.once('idle', function () 
                ...
                for (var i = 0; i < cwbRadioInputs.length; ++i)
				 {
                    cwbRadioInputs[i].addEventListener('change', function(event) 
					{
                        ...
                        for(var j = 0; j < toggleableLayerIds.length; ++j) 
						{
                            ...
                            }			//   end of  if (toggleableLayerIds[j] == inputValue)
                        }		//   end  of  for(var j = 0; j < toggleableLayerIds.length; ++j) 
                    })		//   end of cwbRadioInputs[i].addEventListener('change', function(event) 
                }		//   end of  for (var i = 0; i < cwbRadioInputs.length; ++i)
